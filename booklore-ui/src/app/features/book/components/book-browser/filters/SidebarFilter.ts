...
export function isRatingInRange10(
  rating: number | undefined | null,
  rangeId: string,
): boolean {
  if (rating == null) return false;
...
export function isPageCountInRange(
...
export function isMatchScoreInRange(
...
export function doesBookMatchReadStatus(
  book: Book,
  selected: string[],
): boolean {
...
export class SideBarFilter implements BookFilter {
  constructor(
    private selectedFilter$: Observable<unknown>,
    private selectedFilterMode$: Observable<BookFilterMode>,
  ) {}
    ...
    return combineLatest([this.selectedFilter$, this.selectedFilterMode$]).pipe(
      map(([activeFilters, mode]) => {
        ...
        const filteredBooks = (bookState.books || []).filter((book) => {
          const matches = Object.entries(activeFilters).map(
            ([filterType, filterValues]) => {
              ...
              switch (filterType) {
                case "author":
                  return mode === "or"
                    ? filterValues.some((val) =>
                        book.metadata?.authors?.includes(val),
                      )
                ...
                case "category":
                  return mode === "or"
                    ? filterValues.some((val) =>
                        book.metadata?.categories?.includes(val),
                      )
                    : filterValues.every((val) =>
                        book.metadata?.categories?.includes(val),
                      );
                case "mood":
                  return mode === "or"
                    ? filterValues.some((val) =>
                        book.metadata?.moods?.includes(val),
                      )
                    : filterValues.every((val) =>
                        book.metadata?.moods?.includes(val),
                      );
                case "tag":
                  return mode === "or"
                    ? filterValues.some((val) =>
                        book.metadata?.tags?.includes(val),
                      )
                    : filterValues.every((val) =>
                        book.metadata?.tags?.includes(val),
                      );
                case "publisher":
                  return mode === "or"
                    ? filterValues.some(
                        (val) => book.metadata?.publisher === val,
                      )
                    : filterValues.every(
                        (val) => book.metadata?.publisher === val,
                      );
                case "series":
                  return mode === "or"
                    ? filterValues.some(
                        (val) => book.metadata?.seriesName === val,
                      )
                    : filterValues.every(
                        (val) => book.metadata?.seriesName === val,
                      );
                ...
                case "amazonRating":
                  return filterValues.some((range) =>
                    isRatingInRange(book.metadata?.amazonRating, range),
                  );
                case "goodreadsRating":
                  return filterValues.some((range) =>
                    isRatingInRange(book.metadata?.goodreadsRating, range),
                  );
                case "hardcoverRating":
                  return filterValues.some((range) =>
                    isRatingInRange(book.metadata?.hardcoverRating, range),
                  );
                case "ranobedbRating":
                  return filterValues.some((range) =>
                    isRatingInRange(book.metadata?.ranobedbRating, range),
                  );
                case "personalRating":
                  return filterValues.some((range) =>
                    isRatingInRange10(book.personalRating, range),
                  );
                case "publishedDate":
                  ...
                  return bookYear
                    ? filterValues.some(
                        (val) => val == bookYear || val == bookYear.toString(),
                      )
                    : false;
                case "fileSize":
                  return filterValues.some((range) =>
                    isFileSizeInRange(book.fileSizeKb, range),
                  );
                case "library":
                  return mode === "or"
                ...
                case "pageCount":
                  return filterValues.some((range) =>
                    isPageCountInRange(book.metadata?.pageCount!, range),
                  );
                case "language":
                  return filterValues.includes(book.metadata?.language);
                case "matchScore":
                  return filterValues.some((range) =>
                    isMatchScoreInRange(book.metadataMatchScore, range),
                  );
                case "bookType":
