...
import {
  ResetProgressType,
  ResetProgressTypes,
} from "../../../../../shared/constants/reset-progress-type";
...
import {
  TagColor,
  TagComponent,
} from "../../../../../shared/components/tag/tag.component";
...
export class MetadataViewerComponent implements OnInit, OnChanges {
  ...
  ngOnInit(): void {
      ...
      map((book): MenuItem[] => [
        {
          ...
          icon: "pi pi-sync",
          command: () => {
            this.bookDialogHelperService.openMetadataFetchOptionsDialog(
              book.id
            );
          },
        },
    ...
    this.downloadMenuItems$ = this.book$.pipe(
      filter(
        (book): book is Book =>
          book !== null &&
          ((book.alternativeFormats !== undefined &&
            book.alternativeFormats.length > 0) ||
            (book.supplementaryFiles !== undefined &&
      ...
      switchMap((book) =>
        this.userService.userState$.pipe(
          ...
          map((userState) => {
            const items: MenuItem[] = [
              {
                ...
                icon: "pi pi-upload",
                command: () => {
                  this.bookDialogHelperService.openAdditionalFileUploaderDialog(
                    book
                  );
                },
              },
            ...
            if (
              userState?.user?.permissions.canEmailBook ||
              userState?.user?.permissions.admin
            ) {
              items.push({
                label: "Send Book",
                icon: "pi pi-send",
                items: [
                  ...
                  },
                  {
                    ...
                    icon: "pi pi-cog",
                    command: () => {
                      this.bookDialogHelperService.openCustomSendDialog(
                        book.id
                      );
                    },
                  },
              ...
              if (
                ...
                book.alternativeFormats.forEach((format) => {
                  ...
                  deleteFileItems.push({
                    label: `${format.fileName} (${this.getFileSizeInMB(
                      format
                    )})`,
                    icon: this.getFileIcon(extension),
                    command: () =>
  ...
  read(
    ...
    reader: "ngx" | "streaming" | undefined
  ): void {
        ...
        this.bookService.deleteAdditionalFile(bookId, fileId).subscribe({
          ...
          error: (error) => {
            this.messageService.add({
              severity: "error",
              summary: "Error",
              detail: `Failed to delete additional file: ${
    ...
    this.emailService.emailBookQuick(bookId).subscribe({
      ...
      error: (err) =>
        this.messageService.add({
          ...
          summary: "Error",
          detail:
            err?.error?.message || "An error occurred while sending the book.",
        }),
    });
  }

  assignShelf(bookId: number) {
    this.bookDialogHelperService.openShelfAssignerDialog(
  ...
  onPersonalRatingChange(
    book: Book,
    { value: personalRating }: RatingRateEvent
  ): void {
  ...
  goToPageCountRange(pageCount: number): void {
    const range = pageCountRanges.find(
  ...
  goToFileSizeRange(fileSizeKb: number): void {
    const range = fileSizeRanges.find(
      (r) => fileSizeKb >= r.min && fileSizeKb < r.max
    );
    if (range) {
  ...
  isMetadataFullyLocked(metadata: BookMetadata): boolean {
    const lockedKeys = Object.keys(metadata).filter((k) =>
      k.endsWith("Locked")
    );
    return (
      lockedKeys.length > 0 && lockedKeys.every((k) => metadata[k] === true)
    );
  }
  ...
  getRatingTooltip(
    book: Book,
    source: "amazon" | "goodreads" | "hardcover"
  ): string {
    ...
    switch (source) {
      case "amazon":
        return meta.amazonRating != null
          ? `★ ${meta.amazonRating} | ${
              meta.amazonReviewCount?.toLocaleString() ?? "0"
            } reviews`
          : "";
      case "goodreads":
        return meta.goodreadsRating != null
          ? `★ ${meta.goodreadsRating} | ${
              meta.goodreadsReviewCount?.toLocaleString() ?? "0"
            } reviews`
          : "";
      case "hardcover":
        return meta.hardcoverRating != null
          ? `★ ${meta.hardcoverRating} | ${
              meta.hardcoverReviewCount?.toLocaleString() ?? "0"
            } reviews`
          : "";
      default:
        return "";
  ...
  toggleDateFinishedEdit(book: Book): void {
    if (this.isEditingDateFinished) {
      ...
      this.editDateFinished = book.dateFinished
  ...
  saveDateFinished(book: Book): void {
    ...
    const dateToSave = this.editDateFinished
      ? this.editDateFinished.toISOString()
      : null;
