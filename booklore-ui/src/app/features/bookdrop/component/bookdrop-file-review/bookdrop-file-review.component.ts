...
import {
  BookdropFile,
  BookdropFinalizePayload,
  BookdropFinalizeResult,
  BookdropService,
  FileExtractionResult,
  BulkEditRequest as BackendBulkEditRequest,
  BulkEditResult as BackendBulkEditResult,
} from "../../service/bookdrop.service";
...
export class BookdropFileReviewComponent implements OnInit {
  ...
  @ViewChildren("metadataPicker")
  metadataPickers!: QueryList<BookdropFileMetadataPickerComponent>;
  ...
  ngOnInit(): void {
    ...
    this.appSettings$.pipe(filter(Boolean), take(1)).subscribe((settings) => {
  ...
  set defaultLibraryId(value: string | null) {
    ...
    const selected = this.libraries.find(
      (lib) => lib?.id && String(lib.id) === value
    );
  ...
  get selectedLibraryPaths() {
    ...
    const selectedLibrary = this.libraries.find(
      (lib) => String(lib.id) === this.defaultLibraryId
    );
    return (
      selectedLibrary?.paths.map((path) => ({
        label: path.path,
  ...
  get canFinalize(): boolean {
    if (this.selectAllAcrossPages) {
      return (
        this.totalRecords - this.excludedFiles.size > 0 &&
        this.areAllSelectedHaveLibraryAndPath()
      );
    }
  ...
  get selectedCount(): number {
    if (this.selectAllAcrossPages) {
      ...
      return Object.values(this.fileUiCache).filter((file) => file.selected)
        .length;
    }
  }

  loadPage(page: number): void {
    this.bookdropService
      .getPendingFiles(page, this.pageSize)
  ...
  onLibraryChange(file: BookdropFileUI): void {
    const lib = this.libraries.find(
      (l) => String(l.id) === file.selectedLibraryId
    );
    file.availablePaths =
      lib?.paths.map((p) => ({ id: String(p.id ?? ""), name: p.path })) ?? [];
    file.selectedPathId =
      file.availablePaths.length === 1 ? file.availablePaths[0].id : null;
  }
  ...
  applyLibraryDefaults(): void {
    ...
    this.getSelectedFiles().map((fileUi) => {
      ...
      cachedfUi.availablePaths = selectedPaths.map((path) => ({
        id: String(path.id),
        name: path.path,
      }));
  ...
  selectAll(selected: boolean): void {
    if (selected) {
      ...
      this.excludedFiles.clear();
      Object.values(this.fileUiCache).forEach(
  ...
  confirmReset(): void {
    ...
    this.confirmationService.confirm({
      message:
        "Are you sure you want to reset all metadata changes made to the selected files?",
  ...
  confirmFinalize(): void {
    ...
    this.confirmationService.confirm({
      message: `Are you sure you want to finalize the import of ${selectedCount} file${
        selectedCount !== 1 ? "s" : ""
      }?`,
  ...
  confirmDelete(): void {
    ...
    this.confirmationService.confirm({
      message: `Are you sure you want to delete ${selectedCount} selected Bookdrop file${
        selectedCount !== 1 ? "s" : ""
      ...
      accept: () => {
        ...
        this.bookdropService.discardFiles(payload).subscribe({
          next: () => {
            ...
            this.getSelectedFiles().forEach(
              (file) => delete this.fileUiCache[file.file.id]
            );
  ...
  private finalizeImport(): void {
    ...
    const payload: BookdropFinalizePayload = {
      ...
      defaultLibraryId: this.defaultLibraryId
        ? Number(this.defaultLibraryId)
        : undefined,
      defaultPathId: this.defaultPathId
        ? Number(this.defaultPathId)
        : undefined,
      files,
    };
    ...
    this.bookdropService.finalizeImport(payload).subscribe({
      ...
      error: (err) => {
        ...
        this.messageService.add({
          ...
          detail:
            "Some files could not be moved. Please check the console for more details.",
        });
        this.saving = false;
      },
    });
  ...
  private createMetadataForm(
    original: BookMetadata | undefined,
    bookdropFileId: number
  ): FormGroup {
    return new FormGroup({
      ...
      thumbnailUrl: new FormControl(
        this.urlHelper.getBookdropCoverUrl(bookdropFileId)
      ),
    });
  }
  ...
  openBulkEditDialog(): void {
    ...
    const dialogRef = this.dialogLauncherService.openDialog(
      BookdropBulkEditDialogComponent,
      {
  ...
  openPatternExtractDialog(): void {
    ...
    dialogRef?.onClose.subscribe(
      (result: { results: FileExtractionResult[] } | null) => {
        if (result?.results) {
  ...
  private async applyBulkMetadataViaBackend(
  ...
  private applyBulkMetadataToUI(
    result: BulkEditResult,
    selectedFiles: BookdropFileUI[]
  ): void {
