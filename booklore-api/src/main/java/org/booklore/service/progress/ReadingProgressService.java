...
public class ReadingProgressService {
    ...
    private void syncToKoreader(UserBookProgressEntity progress, BookEntity book, Long userId, ReadProgressRequest request) {
        ...
        EpubProgress epubProgress = request.getEpubProgress();
        if (epubProgress == null || epubProgress.getCfi() == null) {
            return;
        }
        ...
        try {
            String xpointer = epubCfiService.convertCfiToKoreaderProgressXPointer(book.getFullFilePath(), epubProgress.getCfi());
            float koreaderPercent = epubProgress.getPercentage() / 100f;
            ...
            log.info("Synced BookLore progress to KOReader: cfi={}, xpointer={}, percent={}", epubProgress.getCfi(), xpointer, koreaderPercent);
        } catch (Exception e) {
