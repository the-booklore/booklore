...
public interface ReadingSessionRepository extends JpaRepository<ReadingSessionEntity, Long> {
    ...
    @Query(value = """
            SELECT
                sub.month as month,
                sub.hourOfDay as hourOfDay,
                sub.sessionCount as sessionCount,
                sub.totalDurationSeconds as totalDurationSeconds,
                (SELECT c.name
                 FROM reading_sessions rs2
                 JOIN book_metadata bm2 ON bm2.book_id = rs2.book_id
                 JOIN book_metadata_category_mapping bcm2 ON bcm2.book_id = bm2.book_id
                 JOIN category c ON c.id = bcm2.category_id
                 WHERE rs2.user_id = :userId
                 AND YEAR(CONVERT_TZ(rs2.start_time, '+00:00', :tzOffset)) = :year
                 AND MONTH(CONVERT_TZ(rs2.start_time, '+00:00', :tzOffset)) = sub.month
                 GROUP BY c.name
                 ORDER BY COUNT(*) DESC
                 LIMIT 1) as topGenre
            FROM (
                SELECT
                    MONTH(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset)) as month,
                    HOUR(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset)) as hourOfDay,
                    COUNT(*) as sessionCount,
                    SUM(rs.duration_seconds) as totalDurationSeconds
                FROM reading_sessions rs
                WHERE rs.user_id = :userId
                AND YEAR(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset)) = :year
                GROUP BY MONTH(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset)),
                         HOUR(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset))
            ) sub
            ORDER BY sub.month, sub.hourOfDay
            """, nativeQuery = true)
    List<ReadingRhythmDto> findReadingRhythmByUserAndYear(
            @Param("userId") Long userId,
            @Param("year") int year,
            @Param("tzOffset") String tzOffset);
    ...
    @Query(value = """
            SELECT
                ranked.book_id as bookId,
                ranked.bookTitle as bookTitle,
                ranked.dayOffset as dayOffset,
                ranked.totalDurationSeconds as totalDurationSeconds
            FROM (
                SELECT
                    rs.book_id,
                    COALESCE(bm.title, 'Unknown Book') as bookTitle,
                    DATEDIFF(
                        DATE(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset)),
                        (SELECT DATE(CONVERT_TZ(MIN(rs2.start_time), '+00:00', :tzOffset))
                         FROM reading_sessions rs2
                         WHERE rs2.book_id = rs.book_id AND rs2.user_id = :userId)
                    ) as dayOffset,
                    SUM(rs.duration_seconds) as totalDurationSeconds
                FROM reading_sessions rs
                ...
                WHERE rs.user_id = :userId
                AND rs.book_id IN (
                    SELECT top_books.book_id FROM (
                        SELECT rs3.book_id, SUM(rs3.duration_seconds) as total_dur
                        FROM reading_sessions rs3
                        WHERE rs3.user_id = :userId
                        AND YEAR(CONVERT_TZ(rs3.start_time, '+00:00', :tzOffset)) = :year
                        GROUP BY rs3.book_id
            ...
            ORDER BY ranked.book_id, ranked.dayOffset
            """, nativeQuery = true)
    List<ReadingIntensityDto> findReadingIntensityByUserAndYear(
            @Param("userId") Long userId,
            @Param("year") int year,
            @Param("tzOffset") String tzOffset);
    ...
    @Query(value = """
            SELECT
                ...
                COUNT(*) as sessionCount
            FROM reading_sessions rs
            JOIN book_metadata_mood_mapping bmm ON bmm.book_id = rs.book_id
            JOIN mood m ON m.id = bmm.mood_id
            WHERE rs.user_id = :userId
            AND YEAR(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset)) = :year
            AND m.name IN (
                SELECT ranked_moods.name FROM (
                    SELECT m2.name, COUNT(*) as cnt
                    FROM reading_sessions rs2
                    JOIN book_metadata_mood_mapping bmm2 ON bmm2.book_id = rs2.book_id
                    JOIN mood m2 ON m2.id = bmm2.mood_id
                    WHERE rs2.user_id = :userId
                    AND YEAR(CONVERT_TZ(rs2.start_time, '+00:00', :tzOffset)) = :year
                    GROUP BY m2.name
                    ORDER BY cnt DESC
            ...
            GROUP BY m.name, HOUR(CONVERT_TZ(rs.start_time, '+00:00', :tzOffset))
            ORDER BY m.name, hourOfDay
            """, nativeQuery = true)
    List<MoodTimeDto> findMoodTimeByUserAndYear(
            @Param("userId") Long userId,
            @Param("year") int year,
            @Param("tzOffset") String tzOffset);
