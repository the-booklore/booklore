...
public interface BookRepository extends JpaRepository<BookEntity, Long>, JpaSpecificationExecutor<BookEntity> {
    ...
    /**
     * Get distinct series names for a specific library.
     * Returns series names from book metadata, used for Komga series listing.
     */
    @Query("""
            SELECT DISTINCT COALESCE(m.seriesName, m.title, b.fileName) as seriesName
            FROM BookEntity b
            ...
            ORDER BY seriesName
            """)
    List<String> findDistinctSeriesNamesByLibraryId(@Param("libraryId") Long libraryId);

    /**
     * Get distinct series names across all libraries.
     * Returns series names from book metadata, used for Komga series listing.
     */
    @Query("""
            SELECT DISTINCT COALESCE(m.seriesName, m.title, b.fileName) as seriesName
    ...
    /**
     * Find books by series name in a specific library.
     * Optimized query that avoids loading all books.
     */
    @EntityGraph(attributePaths = {"metadata", "shelves", "libraryPath"})
    @Query("""
            ...
            LEFT JOIN b.metadata m
            WHERE b.library.id = :libraryId
            AND COALESCE(m.seriesName, m.title, b.fileName) = :seriesName
            AND (b.deleted IS NULL OR b.deleted = false)
            ORDER BY COALESCE(m.seriesNumber, 0)
            """)
    List<BookEntity> findBooksBySeriesNameAndLibraryId(
            @Param("seriesName") String seriesName,
            @Param("libraryId") Long libraryId);

    /**
     * Count books in a series for a specific library.
     * Used for building series metadata efficiently.
     */
    @Query("""
            SELECT COUNT(b)
            FROM BookEntity b
            ...
            AND (b.deleted IS NULL OR b.deleted = false)
            """)
    long countBooksBySeriesNameAndLibraryId(
            @Param("seriesName") String seriesName,
            @Param("libraryId") Long libraryId);
}
