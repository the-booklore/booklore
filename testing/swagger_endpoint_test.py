#!/usr/bin/env python3
"""
BookLore Swagger/OpenAPI Endpoint Tester
...
Inspired by Stirling-PDF's endpoint testing approach, adapted for BookLore.
...
class TestDataGenerator:
    """Generates test data for different parameter types."""
    
    # Default values for common parameter types
    DEFAULTS = {
        "integer": 1,
        "number": 1.0,
        "string": "test",
    ...
    # BookLore-specific parameter defaults
    BOOKLORE_PARAMS = {
        "bookId": 1,
        "libraryId": 1,
        "userId": 1,
    ...
    def get_value_for_param(cls, name: str, schema: Dict[str, Any]) -> Any:
        """Get a test value for a parameter based on its name and schema."""
        # Check BookLore-specific parameters first
    ...
    def generate_request_body(cls, schema: Dict[str, Any], 
        ...
        """Generate a test request body from a schema."""
        if not schema:
            return {}
        ...
        if schema.get("type") == "object" or "properties" in schema:
            ...
            for prop_name, prop_schema in properties.items():
                # Only include required properties or commonly needed ones
...
class EndpointClassifier:
    """Classifies endpoints for special handling."""
    ...
    # Destructive endpoints (DELETE, POST that creates permanent changes)
    ...
    SKIP_ENDPOINTS = [
        ...
        "/image",
        "/download",
        # OPDS (requires different testing approach)
        "/api/v1/opds",
    ...
    def requires_auth(cls, path: str) -> bool:
        ...
        # Check if it matches auth-required patterns
    ...
    def should_skip(cls, path: str, method: str) -> Tuple[bool, Optional[str]]:
        """Check if an endpoint should be skipped, and why."""
        # Check skip patterns
    ...
    def is_destructive(cls, method: str, path: str) -> bool:
        """Check if an endpoint is destructive."""
    ...
    def get_special_body(cls, method: str, path: str) -> Optional[Dict[str, Any]]:
        """Get special request body for specific endpoints."""
...
class BookLoreApiClient:
    """Client for interacting with BookLore API."""
    ...
    def call_endpoint(self, method: str, path: str, 
        ...
        """
        Call an API endpoint and return (status_code, response_time, error).
        """
...
class SwaggerEndpointTester:
    """Main test runner for Swagger/OpenAPI endpoints."""
    
    # Expected success status codes by method
    SUCCESS_CODES = {
        "GET": {200, 204, 304},
    ...
    # Acceptable "failure" codes that aren't errors
    ACCEPTABLE_CODES = {
    ...
    def prepare_request(self, method: str, path: str, 
        ...
        """Prepare a request by resolving path parameters and generating test data."""
        resolved_path = path
        query_params = {}
        request_body = {}
        
        # Get component schemas for reference resolution
        ...
        for param in parameters:
            ...
            # Only set required parameters or common ones
